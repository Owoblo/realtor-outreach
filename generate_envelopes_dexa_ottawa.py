...
Run generate_letters_dexa_ottawa.py first.

Output:
  output/dexa-ottawa/envelopes/ENVELOPES_TIER1.pdf
  output/dexa-ottawa/envelopes/ENVELOPES_ALL.pdf
"""
...
COPPER = HexColor("#b85718")
SLATE  = HexColor("#253042")
STONE  = HexColor("#6e6459")
BLACK  = HexColor("#19160f")
...
def draw_envelope(c: canvas.Canvas, agent_name: str, brokerage: str,
    ...
    # Recipient block — centred in right zone
    rec_cx = W * 0.60
    rec_cy = H * 0.48
...
def main():
    if not AGENT_CSV.exists():
        print("ERROR: agent_summary.csv not found. Run generate_letters_dexa_ottawa.py first.")
        return
