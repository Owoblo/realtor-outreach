...
Waterloo Region (Kitchener / Waterloo / Guelph / Cambridge)

Reads WKG_Listings_Enriched.csv, ranks all agents by listing count,
generates personalized PDF letters for the top 20%.

Output:
  output/wkg/letters/01_Faisal_Susiwala.pdf   ← top 7 individually (Tier 1 = 10+)
  output/wkg/letters/ALL_TOP_20_PERCENT.pdf   ← all 107 combined
  output/wkg/agent_summary.csv
"""
...
DATA  = Path("/Users/admin/Downloads/sold2move/WKG_Listings_Enriched.csv")
LOGO  = BASE / "logo.jpg"
OUT   = BASE / "output" / "wkg"
LDIR  = OUT / "letters"
EDIR  = OUT / "envelopes"
...
ADDR1       = "55 Cedar Drive"
ADDR2       = "Guelph, ON  N1G 1C4"
PHONE       = "(226) 773-2993"
...
CITIES_STR  = "Kitchener, Waterloo, Guelph, and Cambridge"
...
ML = MR  = 66
MT = MB  = 54
TW       = PW - ML - MR  # 480 pt usable width
...
def clean_agent_name(raw: str) -> str:
    """'Andra Arnold, Salesperson' → 'Andra Arnold'"""
    return re.split(r",\s*(Salesperson|Broker|Sales Rep|Agent)", str(raw))[0].strip()


def normalize_brk_key(s: str) -> str:
    """Produce a stable uppercase key for deduplication."""
    s = re.sub(r"\s+", " ", s.strip().upper())
    s = re.sub(r",?\s*BROKERAGE[-\d]*$", "", s).strip()
    s = re.sub(r",?\s*BROKERAGE$", "", s).strip()
    s = re.sub(r"\.$", "", s).strip()
    return s
...
def analyze(path: Path) -> pd.DataFrame:
    ...
    # Keep Canadian listings with agent names
    df = raw[raw["country"] == "CAN"].copy()
    df = df[df["apify_agent_name"].notna()].copy()
    df["Price"]    = pd.to_numeric(df["unformattedprice"], errors="coerce").fillna(0)
    df["City"]     = df["addresscity"].fillna("Unknown").str.strip()
    df["Address"]  = df["addressstreet"].fillna("").str.strip()
    df["AgentRaw"] = df["apify_agent_name"].str.strip()
    df["BrkRaw"]   = df["apify_brokerage"].fillna("Unknown").str.strip()

    df["AgentName"] = df["AgentRaw"].apply(clean_agent_name)
    df["NormBrk"]   = df["BrkRaw"].apply(normalize_brk_key)

    # Pick canonical brokerage name: most frequent raw form per normalized key
    canonical_map = (df.groupby("NormBrk")["BrkRaw"]
                       .agg(lambda x: x.value_counts().index[0])
                       .to_dict())
    df["Brokerage"] = df["NormBrk"].map(canonical_map)
    ...
    brk = (df.groupby("Brokerage")
             .agg(brk_listings=("AgentName", "count"),
                  brk_agents=("AgentName", "nunique"))
             .reset_index()
...
def build_story(a: dict, total_agents: int) -> list:
    ...
    story.append(Paragraph(
        f"I went through every active residential listing across {CITIES_STR}. "
        "Every agent, every brokerage, every price point, every neighbourhood. "
    ...
    story.append(Paragraph(
        ...
        f"working {REGION}. <b>{below:,} agents</b> are behind you.",
        S["body"]))
    ...
    story.append(Paragraph(
        f"We handle moves all across {REGION} — local, long-distance, full-service "
        "packing, wrapping, and furniture assembly. "
        "Every job carries <b>$2M in liability coverage</b>. "
        "We already work with realtors across the region. "
...
def main():
    ...
    BROKER_CSV = EDIR / "brokerage_addresses.csv"
    if BROKER_CSV.exists():
        ...
        if removed:
            print(f"  Removed {removed} agents (no brokerage address)")
