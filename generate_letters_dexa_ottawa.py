...
Reads ottawa-realtors.csv, ranks agents by listing count,
generates personalized PDF letters for the top 20%.

Output:
  output/dexa-ottawa/letters/01_FirstName_LastName.pdf
  output/dexa-ottawa/letters/ALL_TOP_20_PERCENT.pdf
  output/dexa-ottawa/agent_summary.csv
"""
...
# ── Colours — Dexa brand ──────────────────────────────────────────────────────
COPPER = HexColor("#b85718")   # primary accent
SLATE  = HexColor("#253042")   # dark navy
STONE  = HexColor("#6e6459")   # muted text
BLACK  = HexColor("#19160f")   # ink
...
def analyze(path: Path) -> pd.DataFrame:
    ...
    brk = (df.groupby("Brokerage")
             ...
             .reset_index().rename(columns={"Brokerage": "brokerage"}))
...
def build_story(a: dict, total_agents: int) -> list:
    ...
    story.append(Paragraph(
        ...
        "surrounding area — every agent, every brokerage, every neighbourhood from "
        "Kanata to Orleans, Barrhaven to Rockcliffe. Your name kept coming up.",
        S["body"]))
    ...
    if n >= 5 and hi_price > 0:
        story.append(Paragraph(
            ...
            f"The family spending {fmt(hi_price)} on a home expects the move to feel "
            f"organized, protected, and completely stress-free.",
            S["body"]))
    ...
    story.append(Paragraph(
        "DEXA Movers is a Ottawa-based moving company built around one idea: "
    ...
    story.append(Paragraph("Here's what we'd offer your clients, exclusively through you:", S["body"]))

    bullets = [
        "<b>15% off their move</b> when referred by you",
        "Full-service packing, wrapping, and furniture assembly available",
        "A <b>dedicated move coordinator</b> for every job — one number to call",
        "Real-time updates on move day so nobody is left guessing",
    ...
    story.append(Paragraph(
        "When one of your clients is ready to move, send them our number "
    ...
    story.append(Paragraph(
        "We'll tag the referral under your name, apply the discount, "
    ...
    story.append(Paragraph(
        "<i>P.S. — We already work with top agents in this market. "
        "Happy to share what's been working for them. No strings.</i>",
        S["ps"]))
