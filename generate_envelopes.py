...
Output:
  output/windsor/envelopes/ENVELOPES_TIER1.pdf      ← top 12 (Tier 1)
  output/windsor/envelopes/ENVELOPES_ALL.pdf         ← all top 20%
  output/windsor/envelopes/brokerage_addresses.csv   ← address lookup sheet
"""
...
RET_LINE1 = "1487 Ouellette Avenue"
RET_LINE2 = "Windsor, ON  N8X 1K1"
PHONE     = "(226) 773-2993"
...
# ── Envelope size — standard #10 commercial ───────────────────────────────────
ENV_W = 9.5  * inch   # 684 pt
ENV_H = 4.125 * inch  # 297 pt
PAGE_SIZE = (ENV_W, ENV_H)  # already landscape


def clean_name(agent_name: str) -> str:
    """'Jason Laframboise, Asa, Broker' → 'Jason Laframboise'"""
    part = agent_name.strip().split(",")[0].strip()
    return re.sub(r"\s+", " ", part).strip(".")


def clean_brokerage(name: str) -> str:
    """Fix duplicate 'Brokerage' suffix e.g. '...Brokerage Brokerage'."""
...
def draw_envelope(c: canvas.Canvas, agent_name: str, brokerage: str,
    ...
    """Draw one envelope on the canvas at current page."""
    W, H = ENV_W, ENV_H
    ...
    # Logo (small, above return address text)
    if LOGO.exists():
        lsz = 0.5 * inch
    ...
    c.setFillColor(GRAY)
    for line in [RET_LINE1, RET_LINE2, PHONE]:
        c.drawString(ra_x, ra_y, line)
        ra_y -= 10

    # ── Recipient (centered-right zone) ──────────────────────────────────────
    # Center horizontally from 35% of envelope width to 90%
    rec_cx = W * 0.60
    rec_cy = H * 0.48   # slightly below vertical centre
    ...
    # Address lines — real if available, placeholder if not
    ...
    # ── Canada Post Personalized Mail indicia (upper right) ───────────────────
    if STAMP.exists():
        stamp_w = 1.55 * inch   # ~112pt
        stamp_h = 0.85 * inch   # ~61pt
        stamp_x = W - stamp_w - 0.25 * inch
        stamp_y = H - stamp_h - 0.18 * inch
        c.drawImage(str(STAMP), stamp_x, stamp_y,
                    width=stamp_w, height=stamp_h,
                    preserveAspectRatio=True, mask="auto")

    # Thin top + bottom border lines for visual framing
...
def main():
    if not AGENT_CSV.exists():
        print("ERROR: agent_summary.csv not found. Run generate_letters.py first.")
        return
    ...
    # Load enriched brokerage addresses
    addr_map = {}
    no_addr  = set()
